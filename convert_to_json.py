#!/usr/bin/env python3
"""Convert analysis markdown to structured JSON for frontend consumption."""
...
SYSTEM_PROMPT = f"""你是一个数据转换专家。你的任务是将面试分析报告（Markdown格式）转换为结构化JSON。
...
def convert_md_to_json(md_content: str) -> dict:
    """Call LLM to convert markdown analysis to structured JSON."""
    print(f"  调用 {MODEL} 转换为结构化JSON...")

    response = client.chat.completions.create(
        model=MODEL,
        messages=[
            {"role": "system", "content": SYSTEM_PROMPT},
            {"role": "user", "content": f"请将以下面试分析报告转换为JSON：\n\n{md_content}"},
        ],
        temperature=0.1,
        max_tokens=16000,
    )
...
def main():
    if len(sys.argv) < 2:
        import glob
        files = glob.glob(os.path.join(os.path.dirname(__file__) or ".", "*_analysis.md"))
        if not files:
            print("用法: python convert_to_json.py <analysis.md>")
            sys.exit(1)
    else:
        files = [sys.argv[1]]
    ...
    for md_path in files:
        print(f"\n{'='*60}")
        print(f"转换: {os.path.basename(md_path)}")
        print(f"{'='*60}")
        ...
        output_path = md_path.replace("_analysis.md", "_analysis_data.json")
