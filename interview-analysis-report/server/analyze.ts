...
### Step 2：面试官问题提取
从面试官的所有发言中提取独立问题，用简洁的一句话概括每个问题的核心意思（不超过30字）。注意：
- 一次发言可能包含多个子问题，需拆分
- 跨多个片段的同一个问题需合并
- 记录每个问题的大致时间点
- 面试官的口语化表述需提炼为简洁的问题（如"嗯那个你之前说的那个评测的那个标准是怎么定的" → "评测标准如何制定"）
- **不要遗漏任何实质性问题**

### Step 3：问题分类
对每个问题判定类型：
- **预设**：与候选人前面的回答无关，面试官主动切换话题
...
6. **链条内问题序号递增**：同一链条内的问题编号必须严格按从小到大排列（如 Q2→Q3→Q5），不允许出现 Q5→Q3 这样的倒序
